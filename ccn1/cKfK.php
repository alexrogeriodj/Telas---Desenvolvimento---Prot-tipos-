...
$b = $a['status'];
if ($b == 1) {
$aq = mysql_query("UPDATE `user` SET `credits`=credits-1 WHERE email='$email'");
while($row = mysql_fetch_assoc($exe)){
$credits = $row['credits'];                    
}
$credits = $credits-1;
echo '{"error":0,"msg":"<font color=green><b>Live</b></font> | '.$format.' | [CRE:'.$credits.'] ./BKCN"}';
$wq = mysql_query("INSERT INTO `Live CC`(`fornum`,`gate`,`charge`,`author`,`date`) VALUES ('$format','Gate 1','$charge Cent','$email','$date')");
break;
} if ($b == 2) {
